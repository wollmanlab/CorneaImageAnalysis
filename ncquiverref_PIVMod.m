function ncquiverref_PIVMod(x,y,u,v,units,reftype,refvec,veccol,cont)
...
if col 
 ...
 %hc=colormap(jet(length(cont)));
    %a = makeColorMap([0 0 0],[0.6 0 0.8],[1 1 0],length(cont));
    hc = colormap([0 0 0; plasma(length(cont)-1)]);
 for i=1:length(cont)
...
%        hc(i,:));
 ...
 set(hcb,'YTickLabel',tickn,'YTickMode','manual','YTick',tickpos,'color','w')
