function [infimg, fmap1filt2] = imbedo_ByBrightnessMPProp(Data,Nbins,varargin)
    ...
    for delX = 0:Nbins(1)-1; %<Nbins
        for delY =  0:Nbins(2)-1; %<Nbins
            ...
            if propFlag
                ...
                x = 1:size(testIMG,3);
                x = (x<localZ+3);
                x = permute(repmat(x',1,WindowsizeX,WindowsizeY),[2 3 1]);
                ind = findPlaneByBrightness(testIMG.*x);
            else
                ind = findPlaneByBrightness(testIMG);
            end
...
%     end
    ...
    for ii = 1:Nplanes
        ...
        localMaxProj = max(Data(:,:,max(ii-1,1):min(ii+1,Nplanes)),[],3);
